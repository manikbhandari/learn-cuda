...
__global__ void gpu_convolution(const float* matrix_1_d, unsigned int numRows_1, unsigned int numCols_1, 
    ...
    if(i_1 < numRows_1 && j_1 < numCols_1) {
        int radius_h = numRows_2 / 2;
        int radius_w = numCols_2 / 2;
        float sum = 0.0;
        for(int i_2 = i_1 - radius_h; i_2 <= i_1 + radius_h; i_2++){
            for(int j_2 = j_1 - radius_w; j_2 <= j_1 + radius_w; j_2++) {
                float mat1_el = 0;
                if(i_2 >= 0 && i_2 < numRows_1 && j_2 >= 0 and j_2 < numCols_1) {
                    mat1_el = matrix_1_d[i_2 * numCols_1 + j_2];
                }
                float mat2_el = mask_c[(i_2 - i_1 + radius_h)][(j_2 - j_1 + radius_w)];
                sum += mat1_el * mat2_el;
            }
        }
...
int main() {
    ...
    int numBlocksPerThread_x = (numCols_1 + numThreadsPerBlock.x - 1) / numThreadsPerBlock.x;
    ...
    gpu_convolution <<< numBlocksPerThread, numThreadsPerBlock >>> (matrix_1_d, numRows_1, numCols_1, 
